app: docker run --rm --name resume-app --publish $PORT:3000 --network resume --mount type=bind,source="$PWD",target=/myapp --mount source=resume-gems,target=/usr/local/bundle resume bin/rails server
